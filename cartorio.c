...
int registrar(){ //Função responsável por cadastrar os usuários no sistema	
	...
	file = fopen(arquivo, "w"); //Abrindo o arquivo com a função write, para escrever
...
int main(){
	...
	for(x=1;x=1;){
		...
		scanf("%d", &opcao); //Armazenando a escolha do usuário (leia do portugol)
	
		system("cls"); //Limpando a tela
		
		switch(opcao){ //Início da seleção
			case 1:
				registrar(); //Chamando as funções
			break;
			...
			default:
				printf("Opção indisponível, escolha uma opção válida. \n");
				system("pause");
			break;
		}
	}
